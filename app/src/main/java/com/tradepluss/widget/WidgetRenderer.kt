...
object WidgetRenderer {
    ...
    private fun selectVisibleItems(items: List<AssetItem>): List<AssetItem> {
        ...
        val visible = sortedItems.take(slots.size).toMutableList()

        if (sortedItems.size <= slots.size) return visible

        val gold750 = sortedItems.drop(slots.size).firstOrNull(::isGold750)
        if (gold750 != null && visible.none(::isGold750)) {
            visible[visible.lastIndex] = gold750
            visible.sortByDescending { it.currentValue }
        }
        return visible
    }
